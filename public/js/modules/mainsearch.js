...
function doInteractiveSearch(element) {
    ...
    xhr.open('GET', prefix_api_url+'/api/v1/get_search_pds/?search_str='+element.value, true);
    xhr.send();
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (search) {
		for (let index = 0; index < search.length; index++) {
            ...
            search[index].addEventListener('paste', function (e) {
                e.preventDefault()
                let paste = (e.clipboardData || window.clipboardData).getData("text");
                search[index].value = paste.trim()
                doInteractiveSearch(search[index])
            });
