...
class BascetController extends Controller
{
    ...
    public function has_in_bascet(Request $request) {
        ...
        if (empty($sku))
            return ["sku" => $sku, "count" => 0, "position" => ""];

        $cart_product = Cart::with('tovar_data')->where("carts.session_id", session()->getId())->where('product_sku', $sku)->first();
        return ["sku" => $sku, "count" => (!empty($cart_product))?$cart_product->quentity:0, "position" => $cart_product] ;
    }
